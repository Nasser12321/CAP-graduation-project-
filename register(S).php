...
if(in_array($cvActualExt,$allowed) && $cvError === 0 && $cvSize < 100000000 ) {
 ...
 $sql = "insert into seller (Seller_name, Email, phone_number ,latitude,longitude,cv) 
 VALUES ( '$name', '$email', '$phn' ,'$lat', '$lng','cv/$cvNewName') ";
 ...
 if ( !$result ) {
  ...
  echo "<script> alert(\"You registered successfully, The response will come later\"); </script>";
  echo "<script> window.location.replace('index.php'); </script>";
 }
} 
else {
  echo "<script> alert(\"Wrong registeration\")</script>";
}
 }
}
 mysqli_close($con);
