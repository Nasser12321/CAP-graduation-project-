...
<?php
$uid = $_SESSION['userid'];
 $sql = "Select * from orders, ordered_product WHERE order_ID = OP_OrderId AND UserID = '$uid'
 GROUP BY order_ID ORDER BY order_ID desc";
 $result = mysqli_query($con,$sql);
 if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"The orders list is empty\"); </script>";
    ...
    <br>
    <span style = 'font-size : 20px; font-family : Monospace'>Order number: " .$row['order_ID']. " </span>
    <br>  
     <br> 
    <span style = 'font-size : 20px; font-family : Cursive; color : brown'>Order status: ". $row["order_status"] . " </span>
    <br><br>
 
    <form action = 'orderInfo(U).php' method = POST>
    <input name = 'oid' type = 'hidden' value = ' ". $row['order_ID']." '>
    <button class = 'w3-button w3-yellow' style = ' font-family : Fantasy; height : 50px; font-size : 17px; width : 160px; border : 2px solid black; border-radius : 15px;'  > Order products </button>
    </form>
    <br> <br>
    </div> 
    </div>"
