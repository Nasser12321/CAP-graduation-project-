...
<?php
  ...
  $sid = $_SESSION['id'];
	$sql = "Select * from product, orders, ordered_product where ID = OP_ProductID and
    order_ID = OP_OrderId and SellerID = '$sid' 
    group by ID order by order_ID desc";
	$result = mysqli_query($con,$sql);
  if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"You don't have any previous orders\"); </script>";
	...
	while ($row = mysqli_fetch_array($result)) {
     ...
     <div class='w3-card w3-container w3-animate-zoom w3-hover-gray' style='min-height:460px; margin : 20px'>
		...
		<br>  
         <br>
         <span style = 'font-size : 17.4px; font-family : Monospace'>Ordered quantity: " .$row['OP_quantity']. " </span>
		<br> 
		<span style = 'font-family : Cursive; color : blue'>Order ID:  ". $row["order_ID"] . " </span>
		<br>
