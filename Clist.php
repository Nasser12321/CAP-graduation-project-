...
if (isset($_POST['delete'])) {
	$id = $_POST['cid'];
	$query = "DELETE FROM captain WHERE CaptainID='$id'";
...
<a href="homepage(A).php" class="w3-bar-item w3-button w3-padding-16"><i class="fa fa-home" style="font-size:36px; "></i></a>
  ...
  <br>
  <form  class="navbar-form navbar-left " action= "Clist.php" method="POST">
...
<?php
 ...
 if(isset($_POST["search"])){
	...
	$sql = "Select * from captain WHERE Captain_name like '%$title%' AND Accepted_registration = 1";
	$result = mysqli_query($con,$sql);
  if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"Captains list is empty\"); </script>";
    ...
    <input name = 'cid' id = 'pid' type = 'hidden' value = ' ". $row['CaptainID']." '>
    </form>
        <br> 
    ...
    $sql = "Select * from captain WHERE Accepted_registration = 1";
	$result = mysqli_query($con,$sql);
  if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"Captain list is empty\"); </script>";
    echo "<script> window.location.replace('homepage(A).php'); </script>";
  }else {
    while ($row = mysqli_fetch_array($result)) {
  ...
  <br> <br>
      ...
      <form method = 'POST' action = 'Clist.php'>
      <button class = 'w3-button' type = 'submit' name = 'delete'  style = 'font-family : Fantasy; background-color : red  ; height : 50px; width : 130px; font-size : 17px; border : 2px solid black; border-radius : 15px;'  > Remove </button>
      <input name = 'cid' id = 'sid' type = 'hidden' value = ' ". $row['CaptainID']." '>
      </form>
      <br> 
