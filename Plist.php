...
if (isset($_POST['delete'])) {
  ...
  if($query_run){
  echo "<script> alert(\"The product is removed successfuly\"); </script>";
  }
  else {
...
<a href="homepage(A).php" class="w3-bar-item w3-button w3-padding-16"><i class="fa fa-home" style="font-size:36px; "></i></a>
  ...
  <br>
  <form  class="navbar-form navbar-left " action= "Plist.php" method="POST">
...
<?php
 ...
 if(isset($_POST["search"])){
	...
	$sql = "Select * from product WHERE Name like '%$title%'";
	$result = mysqli_query($con,$sql);
  if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"No matched products\"); </script>";
    echo "<script> window.location.replace('Plist.php'); </script>";
  }
  else {
    ...
    <form method = 'POST' action = 'Plist.php'>
	...
	$result = mysqli_query($con,$sql);
  if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"Products list is empty\"); </script>";
