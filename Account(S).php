...
<div class="w3-row-padding w3-center w3-margin-top">
  ...
  <h2>Profile</h2>
	...
	<form action="Account(S).php" method="post" 
...
"background-color:grey;
  ...
  border-width : 10px">
		...
		$sql = "SELECT * FROM seller WHERE Email='{$_SESSION["useremailS"]}'";
    ...
    <label for="password">Name</label> <br>
			<input type="text" id="name" name="name" placeholder="Name" value="<?php echo $row['Seller_name']; ?>" required>
		</div>
		<div class="inputbox">
    ...
    <div class="inputbox">
		...
		<div class="inputbox">
			...
			<input type="password" id="cpassword" name="cpassword" placeholder="Confirm Password" srequired>
      <br> <br>
    ...
    <form  method="post" action="changeLocationS.php">
...
if (isset($_POST["submit"])) {
  ...
  $phone = mysqli_real_escape_string($con, $_POST["phone"]);
    ...
    $password = sha1($password);
		$sql = "UPDATE seller SET Seller_name='$name', Email='$email', password='$password', 
    phone_number = '$phone'
     WHERE email = '{$_SESSION["useremailS"]}'";
		$result = mysqli_query($con, $sql);
		if ($result) {
  ...
  echo "<script>window.location.replace('homepage(S).php')</script>";    //IMPORTANT
}
mysqli_close($con);
?>
</div>
