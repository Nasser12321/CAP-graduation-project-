...
$captainID = $_SESSION['CaptainID'];
 $sql = "Select * from assigned_order where Captain_ID = '$captainID'";
 $result = mysqli_query($con,$sql);
 if (mysqli_num_rows($result) == 0 ) {
    echo " <script> alert(\"There are no assigned orders\"); </script>";
    ...
    <br>
    <span style = 'font-size : 20px; font-family : Monospace'>Order ID:" .$row['AO_OrderId']. " </span>
    <br>  
     <br> 
    <form action = 'orderInfo(C).php' method = 'POST'>
    <input name = 'orderID' id = 'pid1' type = 'hidden' value = ' ". $row['AO_OrderId']." '>
    ...
    <input name = 'orderID'  type = 'hidden' value = ' ". $row['AO_OrderId']." '>
    <button class = 'w3-button w3-green'  name = 'submit'  style = ' font-family : Fantasy;  height : 50px; font-size : 17px; width : 130px; border : 2px solid black; border-radius : 15px;'  > Submit </button>
    </form>
    <br> <br>
    </div> 
    </div>"
